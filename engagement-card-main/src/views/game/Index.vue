<template style="zoom: 0.85;">
  <div class="game-warp" v-loading="loading">
    <div class="game-box row">
      ...
      >
        <div class="username">{{ user2.nickname }}</div>
        <!-- <div class="username">就是这样的八个字</div> -->
        <div class="user-cards-box column column-center">
          ...
          <!-- <div class="card-box" v-if="!info.isPublic && mode != 'watch'" v-for="(item, index) in user2.cards"
						:data-id="item.id">
						<div class="card-box2 column column-center row-center" style="transform: rotate(90deg)">
							<el-image class="card card2" :src="cardsBack" fit="contain" style="transform: rotate(0)">
							</el-image>
						</div>
					</div> -->
        </div>
        ...
        >
          <div class="release-skip" v-if="isHomewer">スキップ解除</div>
          スキップ状態
        </div>
      </div>
      <!--  中间  -->
      <div class="game-box-min column column-center row-flex">
        ...
        <div
          ...
          :style="user3.sort == playUser ? 'border:4px solid #FF8660' : ''"
          :data-id="user3.id"
        ...
        >
          <div class="username usernamesc">{{ user3.nickname }}</div>
           <!-- <div class="username usernamesc">就是这样的八个字</div> -->
          <div class="user-cards-box row row-center column-center">
            <el-image
              ...
              fit="contain"
              @click="
                changeOtherCard(item, index, $event, 3, user3.cards, 'user3')
              "
            >
            </el-image>
            <el-image
            ...
            <!-- <div class="card-box" v-if="!info.isPublic && mode != 'watch'"
							v-for="(item, index) in user3.cards" :data-id="item.id" style="border: none;background: #fff">
							<div class="card-box2 column column-center row-center" style="border: none;background: #fff">
								<el-image class="card sc" :src="cardsBack" fit="contain">
								</el-image>
							</div>
						</div> -->
          </div>
          <div class="mask" v-show="user3.isSkip">
            <div
              class="release-skip"
              v-if="isHomewer"
        ...
        <div class="game-cards-box row-flex row">
          ...
          <div class="min column">
            ...
            <!--  中间卡牌堆  -->
            <div class="min-cards-box row-flex">
          ...
          <!--  右侧弃牌区  -->
          <div class="right column column-center row-center">
            <div class="cards-lists">
              <div
                class="cards-scroll"
                ref="location4"
      ...
      <div
        ...
        :style="user4.sort == playUser ? 'border:4px solid #FF8660' : ''"
        :data-id="user4.id"
        :data-email="user4.email"
        :data-username="user4.username"
        :data-realname="user4.realname"
        :data-lineStatus="user4.lineStatus"
        :data-isSkip="user4.isSkip"
        :data-sort="user4.sort"
      >
        <div class="username">{{ user4.nickname }}</div>
        <div class="user-cards-box column column-center" style="flex-direction: column-reverse;">
          <el-image
            v-if="info.isPublic || mode == 'watch'"
            :class="item.checked == true ? 'active' : ''"
            v-for="(item, index) in user4.cards"
            :key="index"
            class="card4 cardh"
            :src="item.url"
            :data-id="item.id"
            fit="contain"
            @click="
              changeOtherCard(item, index, $event, 4, user4.cards, 'user4')
            "
          >
          </el-image>
          <el-image
          ...
          <!-- <div class="card-box" v-if="!info.isPublic && mode != 'watch'" v-for="(item, index) in user4.cards"
						:data-id="item.id">
						<div class="card-box2 column column-center row-center" style="transform: rotate(90deg)">
							<el-image class="card" :src="cardsBack" fit="contain" style="transform: rotate(180deg)">
							</el-image>
						</div>
					</div> -->
        </div>
        <div class="mask" v-show="user4.isSkip">
          <div
            class="release-skip"
            v-if="isHomewer"
            @click="cancelSkip(user4.username, user4.id, 4)"
          >
            スキップ解除
          </div>
          スキップ状態
...
export default {
  data() {
    return {
      ...
      // 房间信息
      roomId: "c0e6b1e56ed0141b000650f2ec5232d9", // 房间ID
      play: true, // 正在进行中
  ...
  methods: {
              ...
              switch (location) {
                ...
                case 6:
                  if (that.user6.discard.indexOf(oldCard) == -1) {
                    ...
                    that.user6.discard = [...new Set(that.user6.discard)];
                  }
                  break;
              }
    ...
    doFinish: function () {
      let that = this;
      console.log("usertime",that.userTimes)
      let temp = [];
      temp.push(that.user1);
      temp.push(that.user2);
      temp.push(that.user3);
...
.game-warp .game-box .game-box-left .user-cards-box .card4,
.game-warp .game-box .game-box-right .user-cards-box .card4 {
  width: 40%;
  transform: rotate(-90deg);
}

.game-warp .game-box .game-box-left .user-cards-box .card3,
.game-warp .game-box .game-box-right .user-cards-box .card3 {
  width: 45%;
  transform: rotate(-90deg);
}
...
.game-warp .game-box .game-box-left .user-cards-box .cardh:hover {
...
.game-warp .game-box .game-box-right .user-cards-box .cardh:hover {
  transform: scale(2) rotate(-90deg) !important;
  z-index: 999 !important;
  /* border: 3px solid #1e7578; */
  box-sizing: border-box;
...
.cardl:hover {
  transform: scale(1.4) rotate(90deg) !important;
}
