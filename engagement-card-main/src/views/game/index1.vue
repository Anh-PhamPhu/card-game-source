  ...
  <div class="game-warp" v-loading="loading">
    <div class="game-box row">
        ...
        <div class="game-cards-box row-flex row">
            ...
            <div class="cards-lists" style="display: none">
              ...
              >
                <el-image
                  ...
                  :key="index"
                  class="card"
                  @click.stop="extractCardFromDiscards(item, index, 4, $event)"
                  :src="item.url"
                  fit="contain"
                >
                </el-image>
          ...
          <div class="min column">
            ...
            <div
              ...
              style="position: relative; left: 15%; top: 18%"
            >
                ...
                >
                  <div class="card-box2 column column-center row-center">
                    <el-image
                      class="card"
                      :src="cardsBack"
                      @click.stop="extractCard(item, index)"
                      fit="contain"
                    >
                    </el-image>
                  </div>
            ...
            <!--  中间底部弃牌区  -->
            <div
              class="min-bottom column column-center row-center"
              style="z-index: 999"
            >
              <div class="cards-lists" @click.stop="discardCard(1, $event)">
                <div
            ...
            <div class="cards-lists" style="display: none">
              ...
              >
                <el-image
                  ...
                  :key="index"
                  class="card"
                  @click.stop="extractCardFromDiscards(item, index, 2, $event)"
                  :src="item.url"
                  fit="contain"
                >
                </el-image>
      ...
      <div
        ...
        :style="user3.id == UID ? 'border:3px solid #FF8660' : ''"
        :data-id="user3.id"
      ...
      >
        ...
        <div class="mask" v-show="user2.isSkip">
          <div
            class="release-skip"
            v-if="isHomewer"
            @click.stop="cancelSkip(user3.id, 3)"
          >
            スキップ解除
          </div>
          スキップ状態
      ...
      >
        ...
        >
          <el-image
            ...
            :src="item.url"
            :data-id="item.id"
            @click.stop="changeSelfCard(item, index, $event, 1)"
            fit="contain"
          >
          </el-image>
          <div class="mask" v-show="user1.isSkip">
            <div
              class="release-skip"
              v-if="isHomewer"
              @click.stop="cancelSkip(user1.id, 1)"
            >
              スキップ解除
            </div>
            スキップ状態
    ...
    >
      <div slot="footer" class="dialog-footer btns-box column row-center">
        <el-button class="finish-btn" @click.stop="toFinish()">終了</el-button>
        <el-button class="skip-btn" @click.stop="toSkip()">スキップ</el-button>
      </div>
    </el-dialog>
    <!-- 暂停弹窗 -->
    <el-dialog
    ...
    >
      <div slot="footer" class="dialog-footer btns-box column row-center">
        <el-button class="finish-btn" @click.stop="doFinish()"
          >終了する</el-button
        >
        <el-button class="skip-btn" @click.stop="toGame()">続ける</el-button>
      </div>
    </el-dialog>
    <el-dialog
    ...
    >
      <div class="personnel-box row">
        <div
          ...
          v-if="!item.isHomewer"
          @click.stop="checkUser(item, index, item.checked)"
...
export default {
  ...
  methods: {
    ...
    getsocketData: function (e) {
      ...
      if (data.operateMsg == "替换手牌") {
        that.users2.forEach((item, index) => {
          ...
          console.log("item.userId", item.id);
            ...
            if (that.extractCardInfo.id) {
              ...
              userCards.forEach((card, key) => {
                ...
                if (card.id == data.number) {
                  ...
                  //console.log('删除弃牌的方位', newCard.location)
                  // that.$options.methods.delDiscardCard(newCard, newCard.location,
                  //   that)
                  that.copiedcard = {};
                  that.extracting = false;
                  that.extractCardInfo = {};
    ...
    extractCardFromDiscards: function (
      ...
      // if (event.currentTarget.className == "el-image__inner active") {
      //     event.currentTarget.className = "el-image__inner"
      //     that.discardCardInfo = {}
      // } else {
      //     event.currentTarget.className = "el-image__inner active"
      //     that.discardCardInfo = {
      //         location: location,
      //         index: index,
      //         id: item.id,
      ...
      //     }
      // }
      //console.log('选中的弃牌信息:', that.discardCardInfo)
      let newCard = that.discardCardInfo;
      let temp = [];
      temp.push(that.user1);
      temp.push(that.user2);
    ...
    // 复制对面卡牌
    changeOtherCard(item, index, event, location) {
      let that = this;
      if(that.extracting == true){
         return false;
      }
