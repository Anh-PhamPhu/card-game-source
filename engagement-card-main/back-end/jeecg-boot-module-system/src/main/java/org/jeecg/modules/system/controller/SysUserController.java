...
import org.jeecg.modules.system.dto.SysUserChangePasswordDTO;
import org.jeecg.modules.system.dto.SysUserPasswordDTO;
import org.jeecg.modules.system.dto.SysUserRegisterDTO;
import org.jeecg.modules.system.dto.SysUserUrlDTO;
...
import org.jeecg.modules.system.vo.SysUserUrlVO;
...
public class SysUserController {
    ...
    @ApiOperation(value = "设置用户固定URL",notes = "设置用户URL")
    @RequestMapping(value = "/saveUrl", method = RequestMethod.POST)
    public Result<?> saveUrl(@Valid @RequestBody SysUserUrlDTO object, HttpServletRequest httpServletRequest) {
        ...
        if(object.getProName().equals("基本プラン") && object.getUserUrl().split(",").length > 1){
            return result.error500("此plan只能设置一个url");
        }
        LambdaQueryWrapper<SysUserUrl> lambdaQueryWrapper = new LambdaQueryWrapper();
        lambdaQueryWrapper.eq(SysUserUrl::getUserId,sysUser.getId());
//        lambdaQueryWrapper.eq(SysUserUrl::getProName,object.getProName());
        SysUserUrl sysUserUrl = sysUserUrlService.getOne(lambdaQueryWrapper);

        if(sysUserUrl != null){
            sysUserUrl.setProName(object.getProName());
            sysUserUrl.setUserUrl(object.getUserUrl());
            sysUserUrlService.updateById(sysUserUrl);
            return result.success("设置成功");
        }
        SysUserUrl sysAddUserUrl = new SysUserUrl();
        sysAddUserUrl.setUserId(sysUser.getId());
        sysAddUserUrl.setProName(object.getProName());
        sysAddUserUrl.setUserUrl(object.getUserUrl());
        sysUserUrlService.saveUserUrl(sysAddUserUrl);
        return result.success("设置成功");
    }

    @ApiOperation(value = "根据用户id查询用户固定URL",notes = "根据用户id查询用户固定URL")
    @RequestMapping(value = "/getUserUrl", method = RequestMethod.GET)
    public Result<SysUserUrlVO> getUserUrl(HttpServletRequest httpServletRequest) {
        Result<SysUserUrlVO> result = new Result<>();
        String username = JwtUtil.getUsername(httpServletRequest.getHeader(DefContants.X_ACCESS_TOKEN));
        SysUser sysUser =  sysUserService.getUserByName(username);
        if(ObjectUtils.isEmpty(sysUser)){
            return result.error500("用户为空");
        ...
        lambdaQueryWrapper.eq(SysUserUrl::getUserId,sysUser.getId());
        SysUserUrl sysUserUrl =  sysUserUrlService.getOne(lambdaQueryWrapper);
        if(ObjectUtils.isEmpty(sysUserUrl)){
            result.setCode(200);
            result.setSuccess(true);
            result.setResult(null);
            return result;
        }
        SysUserUrlVO sysUserUrlVO = new SysUserUrlVO();
        BeanUtils.copyProperties(sysUserUrl,sysUserUrlVO);
        sysUserUrlVO.setUserUrlList(Arrays.asList(sysUserUrl.getUserUrl().split(",")));
        result.setCode(200);
        result.setSuccess(true);
        result.setResult(sysUserUrlVO);
        return result;
    }


    @ApiOperation(value = "根据用户id和商品名查询用户固定URL",notes = "根据用户id和商品名查询用户固定URL")
    @RequestMapping(value = "/getUrl", method = RequestMethod.GET)
    public Result<SysUserUrlVO> getUrl(@ApiParam(value = "商品名") @RequestParam(required = true,value = "proName") @NotBlank(message="商品名不能为空") String proName, HttpServletRequest httpServletRequest) {
        Result<SysUserUrlVO> result = new Result<>();
        String username = JwtUtil.getUsername(httpServletRequest.getHeader(DefContants.X_ACCESS_TOKEN));
        SysUser sysUser =  sysUserService.getUserByName(username);
        if(ObjectUtils.isEmpty(sysUser)){
        ...
        SysUserUrl sysUserUrl =  sysUserUrlService.getUrl(sysUser.getId(),proName);
        if(ObjectUtils.isEmpty(sysUserUrl)){
            return result.error500("没有查到这个用户设置的固定url");
        }
        SysUserUrlVO sysUserUrlVO = new SysUserUrlVO();
        BeanUtils.copyProperties(sysUserUrl,sysUserUrlVO);
        sysUserUrlVO.setUserUrlList(Arrays.asList(sysUserUrl.getUserUrl().split(",")));
    ...
    public Result<Boolean> checkOnlyUser(SysUser sysUser) {
        ...
        try {
            ...
            if (user != null) {
                ...
                result.setMessage("このアカウントは既に存在しています");
                return result;
            }
    ...
    public Result<?> changePassword(@RequestBody SysUserPasswordDTO sysUser) {
        ...
        sysUser1.setId(u.getId());
        return sysUserService.changePassword(sysUser1);
    }
    ...
    public Result<?> changPassword(HttpServletRequest httpServletRequest, @RequestBody SysUserChangePasswordDTO json) {
        //String username = json.getString("username");
        String username = JwtUtil.getUsername(httpServletRequest.getHeader(DefContants.X_ACCESS_TOKEN));
        String oldpassword = json.getOldpassword();
        String password = json.getPassword();
        String confirmpassword = json.getConfirmpassword();
...
//                json.put("avatar", item.getSysUser().getAvatar());
    ...
    public Result<JSONObject> userRegister(@RequestBody SysUserRegisterDTO sysUserRegisterDTO) {
        ...
        String realname = sysUserRegisterDTO.getRealname();
        String nickname = sysUserRegisterDTO.getNickname();
        String falsename = sysUserRegisterDTO.getFalsename();
        String address = sysUserRegisterDTO.getAddress();
        Integer sex = sysUserRegisterDTO.getSex();
        Date birthday = sysUserRegisterDTO.getBirthday();
        //未设置用户名，则用email作为用户名
...
//        }
        ...
        if (sysUser1 != null) {
            result.setMessage("このユーザー名は既に登録されています");
            result.setSuccess(false);
            return result;
        }
...
//		}
        ...
        if (oConvertUtils.isNotEmpty(email)) {
            ...
            if (sysUser3 != null) {
                result.setMessage("このメールアドレスは既に使用されています");
                result.setSuccess(false);
                return result;
            }
        }
...
//		}
        ...
        try {
            ...
            user.setFalsename(falsename);
            user.setNickname(nickname);
            user.setAddress(address);
            user.setSex(sex);
            user.setPassword(passwordEncode);
            user.setEmail(email);
            user.setCustomerId(sysUserUrlService.getCustomerId(email));
    ...
    public Result<?> getUserSectionInfoByToken(HttpServletRequest request, @RequestParam(name = "token", required = false) String token) {
        try {
            ...
            if(StringUtils.isNotBlank(sysUserInfoVo.getThirdType())){
                sysUserInfoVo.setYear("1949");
                sysUserInfoVo.setMonth("10");
                sysUserInfoVo.setDay("01");
            }else {
                sysUserInfoVo.setYear(DateUtils.getYear(sysUserInfoVo.getBirthday()));
                sysUserInfoVo.setMonth(DateUtils.getMonth(sysUserInfoVo.getBirthday()));
                sysUserInfoVo.setDay(DateUtils.getDay(sysUserInfoVo.getBirthday()));
            }
...
//
			log.info(" ------ 通过令牌获取信息成功--- " + sysUserInfoVo);
